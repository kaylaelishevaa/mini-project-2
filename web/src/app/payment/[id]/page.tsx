...
export default function PaymentPage({ params }: PaymentPageProps) {
  ...
  async function handlePayTicket(e: React.FormEvent) {
    ...
    try {
      ...
      if (isFree) {
        toast.success("Registration completed!", { autoClose: 1500 });
      } else {
        toast.success("Payment successful!", { autoClose: 1500 });
      }

      // redirect setelah 1.5s
      setTimeout(() => {
  ...
  return (
    ...
    <div className="max-w-2xl mx-auto bg-white p-6 rounded shadow">
      <h1 className="text-2xl font-bold mb-4">
        {isFree ? "Free Registration" : "Payment"} Page for{" "}
        {eventName || "Event"}
      </h1>
      ...
      {!isFree && (
        <>
          ...
          <div className="mb-4">
            ...
            <p>{formatIDR(eventPrice)}</p>
          </div>
        </>
      )}

      {/* Wallet & Points & Coupon Info, tampilkan hanya kalau event berbayar */}
      {!isFree && (
        <>
          <div className="mb-4">
        ...
        {/* Top-up form => hanya muncul jika event berbayar */}
        {!isFree && (
...
//   async function fetchEventDetail() {
//     try {
//       const id = params?.id;  // ambil ID dari route
//       const res = await fetch(`http://localhost:8000/api/v1/events/${id}`);
//       const data = await res.json();
//       if (res.ok && data.data) {
//         setEventName(data.data.name);
...
//       // Jika event gratis, show toast "Registration completed!"
//       // Kalau berbayar, "Payment successful!"
//       if (isFree) {
//         toast.success("Registration completed!", { autoClose: 1500 });
//       } else {
//         toast.success("Payment successful!", { autoClose: 1500 });
...
//   function getDiscountedPrice() {
//     if (isFree) return 0;  // gratis

//     const base = eventPrice * ticketQty;

//     let priceAfterCoupon = base;
//     if (useCoupon && hasCoupon) {
//       // Misal diskon coupon = 10%
//       const disc = Math.floor(priceAfterCoupon * 0.1);
//       priceAfterCoupon = Math.max(priceAfterCoupon - disc, 0);
//     }
...
//           {/* Opsi coupon, redeem points => hanya bila event berbayar */}
//           {!isFree && (
//             <>
//               {hasCoupon && (
//                 <div className="mb-2 flex items-center space-x-2">
