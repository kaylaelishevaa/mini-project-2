...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!meRes.ok) {
        // fallback => misal user belum set role => ke /select-role
        router.push("/select-role");
        // refresh untuk update navbar / UI
        router.refresh();
        return;
      }
      ...
      if (meData.role === "CUSTOMERS") {
        ...
        // jika role belum di-set
        router.push("/select-role");
      }

      // Penting: panggil refresh agar komponen lain (navbar dsb.) langsung update
      router.refresh();
